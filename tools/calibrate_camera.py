...
class Calibrator:
    ...
    def _build(self) -> None:
        ...
        # 2026-08-04 A/B 행 스왑 이후 A행이 입구쪽(아래, y=150)이다. 예전 안내는
        # 스왑 전 값(425, 1050)이라 검증에서 멀쩡한 캘리브를 틀렸다고 오판했다.
        # 기준값 출처: rl/parking_env.py SLOT_COORDINATES
        print("  (예: 슬롯 A1 중심 → 425, 150 / B1 중심 → 425, 1050 근처면 정상)")
        print("  A행 = 아래쪽(입구), B행 = 위쪽(출구)\n")
